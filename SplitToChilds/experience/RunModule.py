...
from SplitToChilds.runtime import FilterParamsAndInput,FilterChildInput,FilterChildParams
from SplitToChilds.transfer import ModelNames
import importlib

def RunWholeOnnxModel(model_name:str,input:dict, shape_dict:dict, driver=drivers.GPU()):
    # (N,3,224,224)——need to set input size for tvm
    mod, params, _ =easy_load_from_onnx(model_name,shape_dict,download_url=OnnxModelUrl.Resnet50,validate_download=False)
...
def RunWholeModelByFunction(model_name:str,input:dict,params:dict,driver=drivers.GPU()):
...
def RunAllChildModelSequentially(model_name:str,input:dict,params:dict,params_dict:dict=None, driver=drivers.GPU()):
