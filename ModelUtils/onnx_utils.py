...
def getInputTensorValueInfo(input_name, model):
    ...
    for name in input_name:
        for params_input in model.graph.input:
            if params_input.name == name or name in params_input.name:
                in_tvi.append(params_input)
        for inner_output in model.graph.value_info:
            if inner_output.name == name or name in inner_output.name:
...
def getOutputTensorValueInfo(output_name, model):
    ...
    for name in output_name:
        out_tvi = [
            inner_output for inner_output in model.graph.value_info if inner_output.name == name]
...
def getInitTensorValue(input_name, model):
    print([init.name for init in model.graph.initializer])
    init_t = []
    for name in input_name:
...
def getConstantNode(model):
    op_name = []
    for i in range(len(model.graph.node)):
        if model.graph.node[i].op_type == "Constant":
            op_name.append(model.graph.node[i])
    return op_name
# 获取节点名列表
