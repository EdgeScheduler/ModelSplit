# this file is create by program.def GoogleNet_0(pre_input=None):    import tvm    from tvm import relay    data_0 = pre_input if pre_input is not None else relay.var("data_0", shape=(1, 3, 224, 224), dtype="float32")    conv1_7x7_s2_b_0 = relay.var("conv1/7x7_s2_b_0", shape=(64, ), dtype="float32")    conv1_7x7_s2_w_0 = relay.var("conv1/7x7_s2_w_0", shape=(64, 3, 7, 7), dtype="float32")    call_0 = relay.nn.conv2d(data_0, conv1_7x7_s2_w_0, strides=[2, 2], padding=[3, 3, 3, 3], channels=64, kernel_size=[7, 7])
    ...
    call_output0 = relay.concatenate(relay.Tuple([call_198, call_199, call_200, call_201]), axis=1)
    return call_output0 if not isinstance(call_output0,tvm.relay.expr.TupleWrapper) else call_output0[0]def GoogleNet_14(pre_input=None):    import tvm    from tvm import relay    call_203 = pre_input if pre_input is not None else relay.var("call_203", shape=(1, 1024, 7, 7), dtype="float32")    call_output0 = relay.nn.avg_pool2d(call_203, pool_size=[7, 7], padding=[0, 0, 0, 0])
    return call_output0 if not isinstance(call_output0,tvm.relay.expr.TupleWrapper) else call_output0[0]def GoogleNet_15(pre_input=None):    import tvm    from tvm import relay    call_204 = pre_input if pre_input is not None else relay.var("call_204", shape=(1, 1024, 1, 1), dtype="float32")    call_output0 = relay.nn.dropout(call_204, rate=0.4)
    return call_output0 if not isinstance(call_output0,tvm.relay.expr.TupleWrapper) else call_output0[0]def GoogleNet_16(pre_input=None):    import tvm    from tvm import relay    call_205 = pre_input if pre_input is not None else relay.var("call_205", shape=(1, 1024, 1, 1), dtype="float32")    call_output0 = relay.reshape(call_205, newshape=[1, 1024])
    return call_output0 if not isinstance(call_output0,tvm.relay.expr.TupleWrapper) else call_output0[0]def GoogleNet_17(pre_input=None):    import tvm    from tvm import relay    call_207 = pre_input if pre_input is not None else relay.var("call_207", shape=(1, 1024), dtype="float32")    call_output0 = relay.nn.batch_flatten(call_207)
    return call_output0 if not isinstance(call_output0,tvm.relay.expr.TupleWrapper) else call_output0[0]def GoogleNet_18(pre_input=None):    import tvm    from tvm import relay    call_208 = pre_input if pre_input is not None else relay.var("call_208", shape=(1, 1024), dtype="float32")    loss3_classifier_b_0 = relay.var("loss3/classifier_b_0", shape=(1000, ), dtype="float32")    loss3_classifier_w_0 = relay.var("loss3/classifier_w_0", shape=(1000, 1024), dtype="float32")    call_209 = relay.nn.dense(call_208, loss3_classifier_w_0, units=1000)
    ...
    call_output0 = relay.subtract(call_211, call_212)
    return call_output0 if not isinstance(call_output0,tvm.relay.expr.TupleWrapper) else call_output0[0]def GoogleNet_19(pre_input=None):    import tvm    from tvm import relay    call_213 = pre_input if pre_input is not None else relay.var("call_213", shape=(1, 1000), dtype="float32")    call_214 = relay.exp(call_213)
